...
### MAGICAL FULLDDSACTIONS
    ...
    act_lst.add("DDS41_trigger", lib_action.DdsAction,
            board="dds41",
            parameters=dict(trigger=True, read_data=False),
            variables=dict(),
            var_formats=dict(),
            categories=["actions", "DDS"],
            comment="")             
...
###TTL###
    ...
    act_lst.add("Shutter DS-Repumper ON", lib_action.DigitalAction,
		board="ttl2",
		parameters=dict(channel=[14], status=[True]),
		categories=["actions", "TTL"])

    act_lst.add("Shutter DS-Repumper OFF", lib_action.DigitalAction,
		board="ttl2",
		parameters=dict(channel=[14], status=[False]),
		categories=["actions", "TTL"])
