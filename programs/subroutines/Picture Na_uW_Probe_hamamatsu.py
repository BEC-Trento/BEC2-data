...
def program(prg, cmd):
    ...
    prg.add(-81100, "AOM Probe Detuning", 0.000)
    prg.add(-5000, "AOM DS + RepumperMOT Amp ", 1000, enable=False)
    prg.add(-3000, "AOM DS + RepumperMOT Freq", 408.00)
    prg.add(-2300, "TTL Picture Hamamatsu  ON")
    prg.add(-2000, "uW ON", functions=dict(time=lambda x: x - cmd.get_var('tpulse'), funct_enable=False))
    prg.add(-1500, "uW OFF")
