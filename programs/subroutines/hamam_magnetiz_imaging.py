...
def program(prg, cmd):
    ...
    prg.add(-250, "TTL uW 2 OFF")
    prg.add(-230, "TTL uW 1 (100W) OFF")
    prg.add(0, "Switch Off Dipole")
    prg.add(20, "magnTOF_imaging", functions=dict(time=lambda x: x + cmd.get_var('magn_tof')))
    prg.add(50, "Oscilloscope Trigger ON")
    prg.add(3000, "TTL ProbeHor ON", functions=dict(time=lambda x: x + cmd.get_var('magn_tof')), enable=False)
    prg.add(3200, "TTL ProbeHor OFF", functions=dict(time=lambda x: x + cmd.get_var('magn_tof')), enable=False)
    prg.add(7500, "TTL ProbeVert ON", functions=dict(time=lambda x: x + cmd.get_var('magn_tof')))
    prg.add(7700, "TTL ProbeVert OFF", functions=dict(time=lambda x: x + cmd.get_var('magn_tof')))
