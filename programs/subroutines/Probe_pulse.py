...
def program(prg, cmd):
    ...
    prg.add(-4200, "AOM Probe Amp ch2 (-)", 70, functions=dict(amplitude=lambda x: cmd.get_var('probevert_amp'), funct_enable=False))
    prg.add(-4200, "AOM Probe Amp ch2 (-)", 30, enable=False)
    prg.add(0, "TTL ProbeHor ON")
    prg.add(0, "TTL ProbeVert ON", enable=False)
    prg.add(20, "TTL ProbeVert OFF", enable=False)
