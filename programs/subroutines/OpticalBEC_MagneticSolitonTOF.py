...
def program(prg, cmd):
    ...
    prg.add(179455560, "F1_imaging", enable=False)
    ...
    prg.add(179671120, "soliton_imaging", functions=dict(time=lambda x: x + cmd.get_var('hold_time')), enable=False)
    ...
    prg.add(181671150, "transfer_m1m2_2", functions=dict(time=lambda x: x + cmd.get_var('hold_time')), enable=False)
    prg.add(181672000, "Probe_pulse_cleaning", functions=dict(time=lambda x: x + cmd.get_var('hold_time')), enable=False)
    prg.add(181672100, "Oscilloscope Trigger ON", functions=dict(time=lambda x: x + cmd.get_var('hold_time')), enable=False)
    ...
    prg.add(181695470, "TTL Picture Hamamatsu  ON", 'emptyprobe', functions=dict(time=lambda x: x + cmd.get_var('hold_time')), enable=False)
    prg.add(181698470, "Probe_pulse_Hamam", functions=dict(time=lambda x: x + cmd.get_var('hold_time')), enable=False)
