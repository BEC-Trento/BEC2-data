...
def program(prg, cmd):
    ...
    prg.add(-1600, "TTL uW 2 OFF")
    prg.add(0, "magnetization_imaging")
    prg.add(250, "Oscilloscope Trigger ON")
    prg.add(20000, "TTL Picture Hamamatsu  ON", 'beating_0', enable=False)
    prg.add(29900, "interferometer", enable=False)
    prg.add(30000, "hamam_twofastpictures", enable=False)
    prg.add(35000, "TTL Picture Hamamatsu  ON", 'beating_1', enable=False)
    prg.add(36100, "transfer_m1to0", enable=False)
    prg.add(36100, "transfer_0to0", enable=False)
    prg.add(36100, "interferometer_2", enable=False)
