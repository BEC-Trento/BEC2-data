...
def program(prg, cmd):
    ...
    prg.add(124541600, "Oscilloscope Trigger ON", functions=dict(time=lambda x: x + cmd.get_var('tpulse')), enable=False)
    prg.add(124542100, "Picture Levit_SG at 0ms - Levit 10 ms 10_2018", functions=dict(time=lambda x: x + cmd.get_var('tpulse')), enable=False)
    prg.add(124542100, "Picture Na_resonant_statedependent", functions=dict(time=lambda x: x + cmd.get_var('tpulse')), enable=False)
    prg.add(124542100, "Picture Na_resonant_hamamatsu")
