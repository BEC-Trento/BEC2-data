...
def program(prg, cmd):
    ...
    prg.add(200000000, "Relay BCompZ Normal")
    prg.add(208772689, "Initialize_Dipole_Off")
    prg.add(208777689, "Set MOT")
    return prg
def commands(cmd):
    import numpy as np
    iters = np.arange(0.2, 0.3, 0.01)
    j = 0
    while(cmd.running):
        compz1 = iters[j]
        cmd.set_var('compz', compz1)
        print('\n-------o-------')
        print('Run #%d/%d, with variables:\ncompz = %g\n'%(j+1, len(iters), compz1))
