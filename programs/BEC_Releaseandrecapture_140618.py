...
def program(prg, cmd):
    ...
    prg.add(291896010, "Dipole_Turnoff_and_Recapture")
    prg.add(292096010, "Config field OFF", functions=dict(time=lambda x: x + cmd.get_var('dt')))
    prg.add(292096010, "Picture MT to Levit at 0ms - Levit 30 ms", enable=False)
    prg.add(292096010, "Picture MT to Levit at 0ms - Levit 20 ms", enable=False)
    prg.add(292096010, "Picture MT to Levit at 0ms - Levit 5 ms", enable=False)
    prg.add(292096010, "Picture MT to Levit at 0ms - Levit 10 ms", enable=False)
    prg.add(292096010, "Picture MT to Levit at 0ms - Levit 50 ms", enable=False)
    prg.add(292096010, "Picture MT to Levit at 0ms - Levit variableTime", enable=False)
    prg.add(292099010, "DAC BCompZ", 0.2400, enable=False)
    prg.add(292099510, "Swich Off Dipole", functions=dict(time=lambda x: x + cmd.get_var('dt')))
    prg.add(292099510, "Picture Na_Shortrepumper_offset", enable=False)
    prg.add(292099510, "Picture Na_2Gdet_offset", enable=False)
    prg.add(292099510, "Picture Na_1Gdet_offset", enable=False)
    prg.add(292104510, "Picture Na_offset", enable=False)
    prg.add(292104550, "Oscilloscope Trigger ON", enable=False)
    prg.add(292304550, "Picture Na_Shortrepumper_offset", functions=dict(time=lambda x: x + cmd.get_var('dt')))
    prg.add(292304550, "Picture Na_3Gdet_offset", enable=False)
    prg.add(292304550, "Picture Na_4Gdet_offset", enable=False)
    prg.add(302304550, "Initialize_Dipole_Off")
    prg.add(302309550, "Set MOT")
    return prg
def commands(cmd):
    import numpy as np
