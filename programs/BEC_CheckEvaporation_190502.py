...
def program(prg, cmd):
    ...
    prg.add(208238050, "Config field OFF")
    prg.add(208239000, "Oscilloscope Trigger ON")
    prg.add(208239050, "Switch Off Dipole")
    prg.add(208239090, "Oscilloscope Trigger OFF")
    prg.add(208240840, "Picture_Mirror_Na_VarProbeDet", functions=dict(time=lambda x: x + cmd.get_var('tof')))
    prg.add(223240840, "Set_MOT", functions=dict(time=lambda x: x +cmd.get_var('tof')))
    prg.add(223241840, "AOM IR Horizontal Amp", 1000)
    prg.add(223241840, "Set_BrightMOT", enable=False)
    return prg
def commands(cmd):
    import numpy as np
