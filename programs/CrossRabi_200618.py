...
def program(prg, cmd):
    ...
    prg.add(303952849, "wait", enable=False)
    prg.add(303952863, "Picture_Levit_2018")
    prg.add(304002863, "TESTBCompY_Picture_Levit_2018", enable=False)
    prg.add(304002863, "Picture MT to Levit at 0ms - Levit 50 ms", enable=False)
    prg.add(304004063, "Swich Off Dipole", functions=dict(time=lambda x: x + cmd.get_var('t1'), funct_enable=False))
    prg.add(304005763, "Config field OFF", functions=dict(time=lambda x: x + cmd.get_var('t1'), funct_enable=False), enable=False)
    prg.add(304007075, "Picture Na_Shortrepumper_offset", enable=False)
    prg.add(304007075, "Picture Na_2Gdet_offset", enable=False)
    prg.add(304007075, "Picture Na_1Gdet_offset", enable=False)
    prg.add(304010075, "Picture Na_offset", functions=dict(time=lambda x: x + cmd.get_var('t1'), funct_enable=False), enable=False)
    prg.add(304010075, "Picture Na_Shortrepumper_offset", enable=False)
    prg.add(304010075, "Picture Na_3Gdet_offset", enable=False)
    prg.add(304010075, "Picture Na_4Gdet_offset", enable=False)
    prg.add(305010075, "Relay BCompZ Normal")
    prg.add(305020075, "IGBT BComp1 field CLOSE")
    prg.add(305030075, "IGBT BComp2 field CLOSE")
    prg.add(320030075, "Initialize_Dipole_Off")
    prg.add(320035075, "Set MOT")
    return prg
def commands(cmd):
    import numpy as np
