...
def program(prg, cmd):
    ...
    prg.add(181545500, "+-1_mixture_preparation", enable=False)
    prg.add(183746500, "Phase-Imprint", functions=dict(time=lambda x: x - cmd.get_var('PI_time')), enable=False)
    prg.add(183746500, "Phase-Imprint-beating", enable=False)
    prg.add(183752800, "beating", enable=False)
    prg.add(183759800, "two_photon_pulse", functions=dict(time=lambda x: x + cmd.get_var('hold_time')), enable=False)
    prg.add(183759800, "two_photon_pulse_DDS", functions=dict(time=lambda x: x - cmd.get_var('uW_pulse')+ cmd.get_var('hold_time')))
...
def commands(cmd):
    import numpy as np
    iters = np.arange(440.000000, 451.000000, 1.000000)
