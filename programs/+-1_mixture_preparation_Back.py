...
def program(prg, cmd):
    ...
    prg.add(10000, "SRS HalfGauss Current ramp", start_t=0.0000, func_args="a=-0.1, b=0.001, duration=0.1, width=0.05", n_points=200, func="(a - b * exp(-duration**2 / width**2)) / (1 - exp(-duration**2 / width**2)) + ((b - a) / (1 - exp(-duration**2 / width**2))) * exp(-(t - duration)**2 / width**2)", stop_t=100.0000, functions=dict(func_args=lambda x: "a={}, b={}, duration=0.1, width=0.05".format(cmd.get_var('SRS_V'), cmd.get_var('SRS_V')-cmd.get_var('arp_amplitude'))))
    prg.add(11000, "TTL uW 1 (100W) ON")
    prg.add(500000, "TTL uW 1 (100W) OFF")
    prg.add(1011100, "wait")
    return prg
