...
def program(prg, cmd):
    ...
    prg.add(302144849, "Landau_Zener_pure_020718", enable=False)
    prg.add(302144849, "Landau_Zener", enable=False)
    prg.add(302144849, "Landau_Zener_NoBComp", enable=False)
    prg.add(304154849, "wait")
    prg.add(304164849, "DAC BComp1", 0.7500, enable=False)
    prg.add(304165349, "DAC BComp2", 0.0000, enable=False)
    prg.add(304215349, "DAC BCompZ", 0.0000, enable=False)
    prg.add(304315349, "Relay BCompZ Normal", enable=False)
    ...
    prg.add(305723349, "uW ON")
    prg.add(305724349, "Synchronize.sub")
    prg.add(305924349, "uW OFF", enable=False)
    prg.add(305934349, "RF Landau-Zener ON")
    prg.add(305954349, "RF Landau-Zener OFF", functions=dict(time=lambda x: x+cmd.get_var('t1'), funct_enable=False))
    prg.add(305957349, "uW OFF")
    prg.add(305962349, "Picture_Levit_2018", functions=dict(time=lambda x: x+cmd.get_var('t1'), funct_enable=False))
    prg.add(305962349, "TESTBCompY_Picture_Levit_2018", enable=False)
    prg.add(305962349, "Picture MT to Levit at 0ms - Levit 50 ms", enable=False)
    prg.add(305963549, "Swich Off Dipole", functions=dict(time=lambda x: x+cmd.get_var('t1'), funct_enable=False))
    prg.add(305965249, "Config field OFF", functions=dict(time=lambda x: x + cmd.get_var('t1'), funct_enable=False), enable=False)
    prg.add(305966561, "Picture Na_Shortrepumper_offset", enable=False)
    prg.add(306046561, "Picture Na_uW_Repump", enable=False)
    prg.add(306046561, "Picture Na_2Gdet_offset", enable=False)
    prg.add(306046561, "Picture Na_1Gdet_offset", enable=False)
    prg.add(306049561, "Picture Na_offset", functions=dict(time=lambda x: x + cmd.get_var('t1'), funct_enable=False), enable=False)
    prg.add(306049561, "Picture Na_Shortrepumper_offset", enable=False)
    prg.add(306049561, "Picture Na_3Gdet_offset", enable=False)
    prg.add(306049561, "Picture Na_4Gdet_offset", enable=False)
    prg.add(307049561, "Relay BCompZ Normal")
    prg.add(307059561, "IGBT BComp1 field CLOSE")
    prg.add(307069561, "IGBT BComp2 field CLOSE")
    prg.add(322069561, "Initialize_Dipole_Off")
    prg.add(322074561, "Set MOT")
    return prg
def commands(cmd):
    import numpy as np
