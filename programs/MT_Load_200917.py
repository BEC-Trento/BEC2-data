...
def program(prg, cmd):
    ...
    prg.add(140616850, "Picture Na_Shortrepumper_offset", functions=dict(time=lambda x: x + cmd.get_var('dt'), funct_enable=False), enable=False)
    prg.add(140616850, "Picture Na_2Gdet_offset")
    prg.add(140616850, "Picture Na_1Gdet_offset", enable=False)
    prg.add(140616850, "Picture Na_offset", enable=False)
    prg.add(140616850, "Picture Na_Shortrepumper_offset", enable=False)
    prg.add(140616850, "Picture Na_3Gdet_offset", enable=False)
    prg.add(140616850, "Picture Na_4Gdet_offset", enable=False)
    prg.add(145616850, "Initialize_Dipole_Off", functions=dict(time=lambda x: x + cmd.get_var('dt'), funct_enable=False))
    prg.add(145621850, "Set MOT", functions=dict(time=lambda x: x + cmd.get_var('dt'), funct_enable=False))
    return prg
def commands(cmd):
    import numpy as np
