...
def program(prg, cmd):
    ...
    prg.add(301979349, "Landau_Zener_NoBComp")
    prg.add(303929349, "wait")
    prg.add(303929363, "Picture_Levit_2018", enable=False)
    prg.add(303979363, "TESTBCompY_Picture_Levit_2018")
    prg.add(303979363, "Picture MT to Levit at 0ms - Levit 50 ms", enable=False)
    prg.add(303980563, "Swich Off Dipole", functions=dict(time=lambda x: x + cmd.get_var('t1'), funct_enable=False))
    prg.add(303982263, "Config field OFF", functions=dict(time=lambda x: x + cmd.get_var('t1'), funct_enable=False), enable=False)
    prg.add(303983575, "Picture Na_Shortrepumper_offset", enable=False)
    prg.add(303983575, "Picture Na_2Gdet_offset", enable=False)
    prg.add(303983575, "Picture Na_1Gdet_offset", enable=False)
    prg.add(303984575, "Picture Na_offset", functions=dict(time=lambda x: x + cmd.get_var('t1'), funct_enable=False), enable=False)
    prg.add(303984575, "Picture Na_Shortrepumper_offset", enable=False)
    prg.add(303984575, "Picture Na_3Gdet_offset", enable=False)
    prg.add(303984575, "Picture Na_4Gdet_offset", enable=False)
    prg.add(304984575, "Relay BCompZ Normal")
    prg.add(319984575, "Initialize_Dipole_Off")
    prg.add(319989575, "Set MOT")
    return prg
def commands(cmd):
    import numpy as np
