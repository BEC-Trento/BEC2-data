...
def program(prg, cmd):
    ...
    prg.add(304199849, "wait", enable=False)
    prg.add(304209849, "Decompress_cross", enable=False)
    prg.add(305709849, "wait", enable=False)
    prg.add(305709863, "Picture_Levit_2018")
    prg.add(305759863, "TESTBCompY_Picture_Levit_2018", enable=False)
    prg.add(305759863, "Picture MT to Levit at 0ms - Levit 50 ms", enable=False)
    prg.add(305761063, "Swich Off Dipole", functions=dict(time=lambda x: x + cmd.get_var('t1'), funct_enable=False))
    prg.add(305762763, "Config field OFF", functions=dict(time=lambda x: x + cmd.get_var('t1'), funct_enable=False), enable=False)
    prg.add(305764075, "Picture Na_Shortrepumper_offset", enable=False)
    prg.add(305764075, "Picture Na_2Gdet_offset", enable=False)
    prg.add(305764075, "Picture Na_1Gdet_offset", enable=False)
    prg.add(305767075, "Picture Na_offset", functions=dict(time=lambda x: x + cmd.get_var('t1'), funct_enable=False), enable=False)
    prg.add(305767075, "Picture Na_Shortrepumper_offset", enable=False)
    prg.add(305767075, "Picture Na_3Gdet_offset", enable=False)
    prg.add(305767075, "Picture Na_4Gdet_offset", enable=False)
    prg.add(306767075, "Relay BCompZ Normal")
    prg.add(306777075, "IGBT BComp1 field CLOSE")
    prg.add(306787075, "IGBT BComp2 field CLOSE")
    prg.add(321787075, "Initialize_Dipole_Off")
    prg.add(321792075, "Set MOT")
    return prg
def commands(cmd):
    import numpy as np
