...
def program(prg, cmd):
    ...
    prg.add(135136050, "Oscilloscope Trigger ON")
    ...
    prg.add(218239050, "Config field OFF", enable=False)
    prg.add(218240000, "Oscilloscope Trigger ON", enable=False)
    prg.add(218240050, "Switch Off Dipole")
    prg.add(218240100, "Config field MT-MOT to Levit", enable=False)
    prg.add(218241100, "DAC MT-MOT Current", 40.0000, enable=False)
    prg.add(218241100, "Config field OFF")
    prg.add(218241140, "Oscilloscope Trigger OFF")
    prg.add(218242890, "Picture_Mirror_Na_VarProbeDet", functions=dict(time=lambda x: x + cmd.get_var('tof')))
    prg.add(218242890, "Picture_Mirror_Levit_VarProbeDet", functions=dict(time=lambda x: x+cmd.get_var('tof')), enable=False)
    prg.add(233242890, "Set_MOT", functions=dict(time=lambda x: x +cmd.get_var('tof')))
    prg.add(233243890, "AOM IR Horizontal Amp", 1000)
    prg.add(233243890, "Set_BrightMOT", enable=False)
    return prg
def commands(cmd):
    import numpy as np
