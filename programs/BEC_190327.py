...
def program(prg, cmd):
    ...
    prg.add(214240000, "Oscilloscope Trigger ON")
    prg.add(214240050, "Switch Off Dipole")
    prg.add(214240100, "Config field MT-MOT to Levit")
    prg.add(214241100, "DAC MT-MOT Current", 40.0000)
    prg.add(214241140, "Oscilloscope Trigger OFF")
    prg.add(214242890, "Picture_Mirror_Na_VarProbeDet", functions=dict(time=lambda x: x + cmd.get_var('tof')), enable=False)
    prg.add(214242890, "Picture_Mirror_Levit_VarProbeDet", functions=dict(time=lambda x: x+cmd.get_var('tof')))
