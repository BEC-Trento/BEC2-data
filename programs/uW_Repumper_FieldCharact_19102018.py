...
def program(prg, cmd):
    ...
    prg.add(272047500, "DAC BCompZ", 0.1000)
    ...
    prg.add(277149000, "Picture HybridTrap to Levit at 0ms - Levit 10 ms 10_2018", enable=False)
    prg.add(277149000, "Picture HybridTrap to Levit at 0ms - Levit 50 ms 10_2018", enable=False)
    prg.add(277150600, "Swich Off Dipole")
    prg.add(277300600, "Picture Na_4Gdet", functions=dict(time=lambda x: x + cmd.get_var('TOF'), funct_enable=False), enable=False)
    prg.add(277300600, "Picture Na_4Gdet_hamamatsu", enable=False)
    prg.add(277300600, "Picture Na_resonant_hamamatsu", enable=False)
    prg.add(277300600, "Picture Na_uW_Probe_hamamatsu")
    prg.add(277300600, "Picture Na_2Gdet", enable=False)
    prg.add(277300600, "Picture Na_1Gdet", enable=False)
    prg.add(277300600, "Picture Na_0Gdet", enable=False)
    prg.add(282300600, "Set_MOT")
    prg.add(282300600, "Set_BrightMOT", enable=False)
    return prg
def commands(cmd):
    import numpy as np
    iters = np.arange(1.5, 5, 0.1)
    j = 0
    while(cmd.running):
        tpulse1 = iters[j]
